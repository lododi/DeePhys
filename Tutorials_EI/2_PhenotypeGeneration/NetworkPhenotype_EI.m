...
pattern_rg.removeLowFRUnits(0.5);
...
network_features =[]; % ["Regularity","Burst","Catch22","GraphFeatures"] or "all"
...
cluster_var = ["E_IRatio","DIV"]; % Can also be a list of several metadata vars

nodeSz = 20; % Scatter point size
mapSz = 500; % Size of the plot
sigma = 10; % Spread of the background coloring
%plot_colors = viridis(100);
%cmap = [plot_colors(90,:);plot_colors(75,:);plot_colors(50,:);plot_colors(35,:);plot_colors(20,:)];
cmap = colormap(prism); % Adjust for the number of conditions you want to plot simultaneously

figure("Color","w");
[cluster_idx, group_labels_comb] = pattern_rg.plot_true_clusters_EI(dr_level, dr_method, cluster_var, ...
    cmap=[],nodeSz=nodeSz, mapSz=mapSz, sigma=sigma,plot_centroid=true);
