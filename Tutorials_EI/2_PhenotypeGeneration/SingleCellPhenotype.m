...
rg_params.Selection.Inclusion = {{"DIV",[25,32,49,55,62,69,76,83]},{'E_IRatio',["0:100","100:0"]}}; %Cell array of cell arrays with fieldname + value // empty defaults to including all recordings
rg_params.Selection.Exclusion = {}; %Cell array of cell arrays with fieldname + value
BT_rg = RecordingGroup(mearec_array, rg_params);
...
BT_rg.reduceDimensionality(dr_level, dr_method, n_dims, unit_features, network_features, feature_names, useClustered,...
...
%%
% BT_rg_inh = BT_rg;
% BT_rg_inh.Units = BT_rg_inh.Units(full_umap_labels==2);


%% Clusters in one plot

grouping_var = ["E_IRatio","DIV"];
figure("Color","w");
[cluster_idx, group_labels_comb] = BT_rg.plot_true_clusters_EI(dr_level, dr_method, grouping_var,plot_centroid=true,cmap=colormap(prism));
set(gca,'Color','w')
%%
    nClusters = max(cluster_idx);
    V = viridis(256);

    % choose a base color from viridis (avoid extreme yellow)
    baseColor = V(200,:);   % adjust index (150–220 works well)

    % create light -> dark scaling factors
    factors = linspace(1.4, 0.6, nClusters)';  
    % >1 = lighter, <1 = darker

    % scale base color
    cmap = baseColor .* factors;

    % make sure RGB stays in valid range
    cmap(cmap > 1) = 1;
    cmap(cmap < 0) = 0;
...
BT_rg.plot_dimensionality_reduction(BT_rg.DimensionalityReduction.Unit.UMAP.Reduction, cluster_idx, group_labels_comb,);
...
clust_param = 1; 
BT_rg.clusterByFeatures(dr_method,dr_level,clust_method,clust_param);

% Only relevant for louvain clustering
n_clust = max(BT_rg.Clustering.Unit.(clust_method).Index);
fprintf('Found %i clusters \n', n_clust)

%% Visualize clustering
% Coordinates of dimensionality reduction
reduction = BT_rg.DimensionalityReduction.Unit.(dr_method).Reduction;
cluster_idx = BT_rg.Clustering.Unit.(clust_method).Index;
...
[value_array, group_labels_comb] = BT_rg.plot_cluster_densities(dr_level, dr_method, cluster_idx, ...
...
[value_array, group_labels_comb] = BT_rg.plot_cluster_densities(dr_level, dr_method, metadata_var, ...
    pooling_vals, n_bins, smoothing_factor);

figure("Color","w")
BT_rg.plot_cluster_shifts(value_array, group_labels_comb)
...
BT_rg.plot_unit_cluster_heatmap(cluster_idx,feature_names);
...
BT_rg.plot_unit_cluster_features(cluster_idx,feature_names, cmap);
...
pooling_vals = {};
figure('Color','w');
pattern_ratios = BT_rg.plot_cluster_proportions(metadata_var, pooling_vals, clust_method, colors);
