...
root_path = "/net/bs-filesvr02/export/group/hierlemann/intermediate_data/Maxtwo/mpascual/Dup15/spikesorting"; %Root path
path_logic = {'T00*','2*','w*','sorter_output','qc_output'}; %Variable parts
...
mearec_array = recording_array_from_single_files(sorting_path_list, rm_con);
...
% them easily here. However, use with caution as this might

unit_threshold = 5; % Minimum number of units
mearec_array_combined = remove_low_unit_recordings(mearec_array_combined, unit_threshold);
...
rg_params.Selection.Inclusion = {{"Patterning","CTRL"}}; %Cell array of cell arrays with fieldname + value // empty defaults to including all recordings {"ChipID","T003062_05"},{"ChipID","T003062_14"},{"ChipID","T003062_15"},{"ChipID","T003062_16"},{"ChipID","T003107_09"},{"ChipID","T003107_10"},{"ChipID","T003107_03"},{"ChipID","T003107_01"},{"ChipID","T003107_02"},{"ChipID","T003107_12"}
...
% WaveMAP algorithm.
unit_features = ["Catch22"]; %"ReferenceWaveform","WaveformFeatures","ActivityFeatures","RegularityFeatures","GraphFeatures","Catch22" or "all"

% Network features are only relevant for dr_level = Recording
network_features =["Catch22"]; % ["Regularity","Burst","Catch22","GraphFeatures"] or "all"
...
cluster_var = ["DIV"]; % Can also be a list of several metadata vars

nodeSz = 20; % Scatter point size
mapSz = 500; % Size of the plot
sigma = 10; % Spread of the background coloring
cmap = othercolor('cbrewer',8); % Adjust for the number of conditions you want to plot simultaneously

figure("Color","w");
[cluster_idx, group_labels_comb] = pattern_rg.plot_true_clusters(dr_level, dr_method, cluster_var, ...
    cmap=cmap,nodeSz=nodeSz, mapSz=mapSz, sigma=sigma, plot_centroid = true);
...
pooling_vals = {{{"14","15","16"},{"21","22"},{"28","29"},{"35","36"},{"43","42"},{"49","50"}}};
% It looks weird, but we really need 3 nested cell arrays :)
% number of metadata variables * number of groups * conditions in group

figure("Color","w");
[cluster_idx, group_labels_comb] = pattern_rg.plot_true_clusters(dr_level, dr_method, cluster_var, ...
    cmap=cmap,nodeSz=nodeSz, mapSz=mapSz, sigma=sigma,pooling_vals=pooling_vals,  plot_centroid = true);
legend(["week 2","week 3","week 4","week 5","week 6","week 7"])
...
rg_params.Selection.Inclusion = {{"Patterning","CTRL"}}; %Cell array of cell arrays with fieldname + value // empty defaults to including all recordings
...
figure('Color','w');
confusionchart(conf_mat, ["100:0","75:25","50:50","0:100"])
