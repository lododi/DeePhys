...
root_path = "/net/bs-filesvr02/export/group/hierlemann/intermediate_data/Maxtwo/lododi2/Neuropixel_Dataset"; % INSERT YOUR OWN PATH HERE
path_logic = {'25*','T*','we*','*'}; 
...
rm_con = false; %If true, removes CCG data to save memory
np_array = recording_array_from_single_files(sorting_path_list, rm_con);
...
% results!
unit_threshold = 5; % Minimum number of units
np_array = remove_low_unit_recordings(np_array, unit_threshold);
...
np_rg = RecordingGroup(np_array, rg_params);
...
n_dims = 10; %Number of output dimensions
...
% WaveMAP algorithm.
unit_features = ["ReferenceWaveform","ACG"];%"ReferenceWaveform","WaveformFeatures","ActivityFeatures","RegularityFeatures","GraphFeatures","Catch22"
feature_names = [];
...
np_rg.reduceDimensionality(dr_level, dr_method, n_dims, unit_features, network_features, feature_names, useClustered,...
...
grouping_var = ["Depth"];
figure("Color","w");
[cluster_idx, group_labels_comb] = np_rg.plot_true_clusters(dr_level, dr_method, grouping_var);
...
np_rg.plot_dimensionality_reduction(np_rg.DimensionalityReduction.Unit.UMAP.Reduction, cluster_idx, group_labels_comb);
...
clust_param = 1; 
np_rg.clusterByFeatures(dr_method,dr_level,clust_method,clust_param);

% Only relevant for louvain clustering
n_clust = max(np_rg.Clustering.Unit.(clust_method).Index);
fprintf('Found %i clusters \n', n_clust)

%% Visualize clustering
% Coordinates of dimensionality reduction
reduction = np_rg.DimensionalityReduction.Unit.(dr_method).Reduction;
cluster_idx = np_rg.Clustering.Unit.(clust_method).Index;
...
[value_array, group_labels_comb] = np_rg.plot_cluster_densities(dr_level, dr_method, cluster_idx, ...
...
[value_array, group_labels_comb] = np_rg.plot_cluster_densities(dr_level, dr_method, metadata_var, ...
    pooling_vals, n_bins, smoothing_factor);

figure("Color","w")
np_rg.plot_cluster_shifts(value_array, group_labels_comb)
...
np_rg.plot_unit_cluster_heatmap(cluster_idx,feature_names);
...
np_rg.plot_unit_cluster_features(cluster_idx,feature_names, cmap);
...
pooling_vals = {};
figure('Color','w');
pattern_ratios = np_rg.plot_cluster_proportions(metadata_var, pooling_vals, clust_method, colors);
...
cluster_idx = np_rg.Clustering.Unit.louvain.Index; % Choose your clustering of choice
feature_groups = "all";
grouping_var = [];      % Only relevant when the recording was concatenated and units can be tracked (e.g. drug response)
grouping_values = [];   % Values corresponding to grouping_var (use nan if you want to use all available values)
...
result = np_rg.classifyClusteredUnits(cluster_idx, feature_groups, grouping_var, grouping_values, ...
    feature_names, normalization, normalization_var, N_hyper, K_fold, tolerance);

%% Evaluate classification results
% For a basic assessment we can run 
[metrics_special, train_acc, avg_pred_imp, conf_mat] = np_rg.assessClassifier(result);
...
pooling_vals = {};
metadata_var = "Patterning";
[group_idx, group_labels] = np_rg.combineMetadataIndices("Unit", metadata_var, pooling_vals);
...
result = np_rg.applyClusteredUnitClassifier(train_cluster_idx, train_idx, test_idx, feature_groups, ...
...
%%
mlp = true;
sel_units_new = np_rg.Units;
feature_types = ["ReferenceWaveform","ACG"];
umap_normalization_var = "DIV";
dr_input_table_new = MEArecording.getUnitFeatures(sel_units_new, feature_types);
[~, X_test_new, ~] = prepareInputMatrix(np_rg, dr_input_table_new, sel_units_new, umap_normalization_var, [], []);

% Apply trained UMAP + MLP
[Y_pred_new, ~, test_umap_new, test_reduction_new] = supervised_umap_ld([], [], feature_names, X_test_new, mlp);

% Plot embedding and predicted labels
figure;
scatter(test_reduction_new(:,1), test_reduction_new(:,2), 10, Y_pred_new, 'filled');
xlabel('UMAP1'); ylabel('UMAP2'); title('Predicted labels on new dataset');