classdef MEArecording < handle
    ...
    methods (Hidden)
        ...
        function parseMetadata(obj, metadata)
            ...
            elseif ~isfield(metadata,"InputPath")
                obj.Metadata = obj.retrieve_DB_metadata(metadata);  %% LD change
                disp('Import metadata from DB')
            ...
            elseif isfield(metadata,"LookupPath") && ~isempty(metadata.LookupPath)
                ...
                obj.Metadata = obj.retrieveMetadata(metadata); %% LD change
                warning("Used metadata provided, no import")
            end

            if isfield(obj.Metadata,"RecordingDate") && ~isempty(obj.Metadata.RecordingDate) && isfield(obj.Metadata,"PlatingDate") && ~isempty(obj.Metadata.PlatingDate) && ~isfield(obj.Metadata, "DIV")
        ...
        function [firing_rates, unit_spike_times] = calculateFiringRates(obj, N_units)
            ...
            % old_active_ids = find(obj.Spikes.hasSpikes); %%%% Start change: LD change to accomodate recordings where some units have no spikes
            % new_id_map = zeros(1,max(spike_units));
            % new_id_map(old_active_ids) = 1:N_units;
            % spike_units_new = new_id_map(spike_units);  %%%% End change: LD change to accomodate recordings where some units have no spikes
            unit_spike_times = arrayfun(@(x) spike_times(spike_units == x), 1:N_units,'un',0); %spike_units_new instead of spike_units
    ...
    methods (Static)
        ...
        function metadata_struct = retrieveMetadata(metadata)
            ...
            if ~isfield(metadata_struct,'RecordingDate')
                ...
                metadata_struct.RecordingDate = double(recording_date);
            else
            ...
            if ~isfield(metadata_struct,'PlateID')
                ...
                plate_number = plate_number(end-4:end);
                assert(~isnan(str2double(plate_number)),'Wrong PathPartIndex(2) / PlateID') %Check if plateID date is a number
            else
                plate_id = metadata.PlateID;
            end

            if ~isfield(metadata_struct,'WellID')
                well_id = char(path_parts(metadata.PathPartIndex(3)));
                well_id = sprintf('%02d', str2double(well_id(end-1:end)) + 1);
                assert(~isnan(str2double(well_id)),'Wrong PathPartIndex(3) / WellID') %Check if wellID is a number
            else
                well_id = metadata.WellID;
            end
            ...
            for ids = 1:size(info_table,1)
                chip_ids = strtrim(string(strsplit(info_table.Chip_IDs{ids},",")));%LD problematic with Cgen-single-unit lookup (because there is no recordingdate column)
                if contains(chip_id,chip_ids) && double(recording_date) == info_table.("RecordingDate")(ids) % LD problematic with Cgen-single-unit lookup (because there is no recordingdate column)
                    ...
                    break
                end % LD problematic with Cgen-single-unit lookup (because there is no recordingdate column)
            end
        end

        function metadata_struct = retrieve_DB_metadata(metadata) %% new LD
            arguments
                metadata struct
            end
            ...
            openProject('~/scripts/myrepos/adhd/Adhd.prj');

            DB = recordingsDB();

            DB.filter('Main','UniqueID',metadata.UniqueID);

            for iVar = 1:length(metadata.Variables)
                if metadata.Variables(iVar) == "WellID"
                    metadata_struct.(metadata.Variables(iVar)) = compose("%02d", DB.Selected.Main.(metadata.Variables(iVar)));
                elseif isdatetime(metadata.Variables(iVar))
                    metadata_struct.(metadata.Variables(iVar)) = string(DB.Selected.Main.(metadata.Variables(iVar)),'yyMMdd');
                else
                    metadata_struct.(metadata.Variables(iVar)) = DB.Selected.Main.(metadata.Variables(iVar));
                end
            end
            % metadata_struct.RecordingDate = string(DB.Selected.Main.Date,'yyMMdd');
            % metadata_struct.PlateID = DB.Selected.Main.PlateID;
            % metadata_struct.WellID = compose("%02d", DB.Selected.Main.WellID);
            % metadata_struct.PlatingDate = string(DB.Selected.Main.PlatingDate,'yyMMdd');
            % metadata_struct.DIV = DB.Selected.Main.DIV;
            % metadata_struct.Patterning = DB.Selected.Main.CellLine;
            % metadata_struct.Density = DB.Selected.Main.Condition2;
            % metadata_struct.Chip_IDs = DB.Selected.Main.ChipID;
        ...
        function feat_table = getUnitFeatures(obj, unit_features)
            ...
            for f = 1:length(unit_features)
                ...
                if unit_features(f) == "ReferenceWaveform"
                    % if length(unit_array(1,1).ReferenceWaveform)<100
                    %
                    % else
                    %     aligned_wf =
                    % end
    ...
    methods
        ...
        function [ccgs,t] = calculateFullCCG(obj,args) %Use only if the recording was split from a longer/concatenated recording
            arguments
                obj MEArecording
                args.binsize (1,1) double = 0.0005 %0.0005
                args.duration (1,1) double = 0.1 %0.1
            end
            
            spike_times = readNPY(fullfile(obj.ParentRecordingPath,'spike_times.npy'));  %% fileparts temp for non qc_output files like neuropixel dataset
            spike_templates = readNPY(fullfile(obj.ParentRecordingPath,'spike_templates.npy')); %% fileparts temp for non qc_output files like neuropixel dataset
            ...
            % spike_times = readNPY(fullfile(fileparts(obj.ParentRecordingPath),sprintf('well%03d', str2double(obj.Metadata.WellID)-1),'spike_times.npy'));  %% fileparts temp for non qc_output files like neuropixel dataset
            % spike_templates = readNPY(fullfile(fileparts(obj.ParentRecordingPath),sprintf('well%03d', str2double(obj.Metadata.WellID)-1),'spike_templates.npy')); %% fileparts temp for non qc_output files like neuropixel dataset
            % good_ids = [obj.Units.TemplateID];
            % spike_times = double(spike_times(ismember(spike_templates,good_ids)))/obj.RecordingInfo.SamplingRate;
            % spike_templates = spike_templates(ismember(spike_templates,good_ids));
            % spike_templates = findgroups(spike_templates);
