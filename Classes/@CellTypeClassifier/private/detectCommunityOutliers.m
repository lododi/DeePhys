function det = detectCommunityOutliers(X_feat, responsive_local, subset_responsive, ...
        subset_global_idx, reduction, N_all, n_neighbors, ...
...
[inh_comm_ids, comm_resp_frac, comm_qvals] = identifyInhibitoryCommunities( ...
    best_M, resp_global_all, p_comm.CommunityFDRLevel);
fprintf('Inhibitory communities: %d/%d (hypergeometric enrichment, FDR q<%.3f)\n', ...
    numel(inh_comm_ids), n_comm, p_comm.CommunityFDRLevel);
...
if frac_resp_in_inh < p_comm.CommunityFallbackThreshold
    ...
    det.community_qvals = comm_qvals(:)';
