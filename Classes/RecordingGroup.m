classdef RecordingGroup < handle
    ...
    methods (Hidden)
        ...
        function add_WIV(rg) %% LD 20251208
            for r = 1:length(rg.Recordings)
                DIV = rg.Recordings(r).Metadata.DIV;
                WIV = floor((DIV - 2) / 7) + 1; % Round by defect with the exception of 1 day before
                rg.Recordings(r).Metadata.WIV = WIV;
                rg.Recordings(r).saveObject();
            end
        end
    ...
    methods (Static)
        ...
        function aligned_wf = alignWaveforms(unit_array, options)
            arguments
                unit_array Unit
                options.interp_factor double = []
                options.shift double = 1
            end
            if isempty(options.interp_factor) & height(unit_array(1,1).ReferenceWaveform)<100
                interp_factor = 120000/unit_array(1).MEArecording.RecordingInfo.SamplingRate;
            elseif height(unit_array(1).ReferenceWaveform)>100
                aligned_wf = [unit_array.ReferenceWaveform];
                return
            else
        ...
        function [reduction, cmap] = plot_cluster_outlines(reduction, cluster_idx, ax, plot_centroid, nodeSz, mapSz, sigma, cmap)
            ...
            if sigma == 0 %% LD on 02032026
                ...
                for i = 1:max(cluster_idx)
                    ...
                    scatter(coori(:,1),coori(:,2), ...
                        nodeSz, ...
                        repmat(cmap(i,:),sum(idx),1), ...
                        'o','filled', ...
                        'MarkerEdgeColor','k');
                end
    ...
    methods
        ...
        function removeLowFRUnits(rg, minFR) %% LD 20260205
            arguments
                rg RecordingGroup
                minFR (1,1) {isnumeric}
            end
            % Filter the Units
            units_FR = zeros(1,length(rg.Units));
            for iUnit = 1:length(rg.Units)
                units_FR(iUnit) = height(rg.Units(iUnit).SpikeTimes)/rg.Units(iUnit).MEArecording.RecordingInfo.Duration;
            end

            rg.Units = rg.Units(units_FR > minFR);

            % Filter the Units within the Recordings
            for iRec = 1:length(rg.Recordings)
                mask_FR = zeros(1, length(rg.Recordings(iRec).Units));
                for iUnit = 1:length(rg.Recordings(iRec).Units)
                    mask_FR(iUnit) = height(rg.Recordings(iRec).Units(iUnit).SpikeTimes)/rg.Recordings(iRec).RecordingInfo.Duration;
                end
                rg.Recordings(iRec).Units = rg.Recordings(iRec).Units(mask_FR > minFR);
            end

            % Filter the Units within the Cultures (being tested)
            for iCul = 1:length(rg.Cultures)
                for iRec = 1:length(rg.Cultures{iCul})
                    mask_FR = zeros(1, length(rg.Cultures{iCul}(iRec).Units));
                    for iUnit = 1:length(rg.Cultures{iCul}(iRec).Units)
                        mask_FR(iUnit) = height(rg.Cultures{iCul}(iRec).Units(iUnit).SpikeTimes)/rg.Cultures{iCul}(iRec).RecordingInfo.Duration;
                    end
                    rg.Cultures{iCul}(iRec).Units = rg.Cultures{iCul}(iRec).Units(mask_FR > minFR);
                end
            end
        end
            ...
            switch method
                case "UMAP"
                    ...
                    [reduction, umap, clusterIdentifiers, extras] = run_umap(norm_data,'n_components',n_dims,'n_neighbors',n_neighbors,'min_dist',1,'cluster_detail','adaptive','spread',5,'sgd_tasks',20,...
                        'verbose','none','color_file',color_file,'method','Java'); %Torsten
        ...
        function [cluster_idx, group_labels_comb] = plot_true_clusters(rg, level, method, grouping_var, options)
            ...
            ax = gca;
            if grouping_var == "E_IRatio"
                E_IRatio_order = [1,3,4,5,2];
                E_IRatio_map = [1,5,2,3,4];
                group_labels_comb = group_labels_comb(E_IRatio_order);
                cluster_idx = E_IRatio_map(cluster_idx)';
            end
        ...
        function [cluster_idx, group_labels_comb] = plot_true_clusters_EI(rg, level, method, grouping_var, options)
            arguments
                rg RecordingGroup
                level string = "Unit" %Unit, Recording, Culture
                method string = "UMAP" %dim reduction method: "UMAP","PCA"
            ...
            metadata_object = rg.DimensionalityReduction.(level).ObjectGroup;
            [cluster_idx, group_labels_comb] = combineMetadataIndices(rg, metadata_object, grouping_var, options.pooling_vals);
            %% ===== Stark-contrast colormap (strong light/dark separation) =====
            if isempty(options.cmap)

                nClusters = max(cluster_idx);

                base_map = viridis(100);

                % Major anchors (green-like, blue-like)
                anchor_idx = [90, 20];
                major_anchor_colors = base_map(anchor_idx,:);

                n_major = 2;

                cmap_full = zeros(nClusters,3);

                clusters_per_major = ceil(nClusters / n_major);

                for major_id = 1:n_major

                    anchor_rgb = major_anchor_colors(major_id,:);
                    anchor_hsv = rgb2hsv(anchor_rgb);

                    cluster_range = ((major_id-1)*clusters_per_major + 1) : ...
                        min(major_id*clusters_per_major, nClusters);

                    n_sub = numel(cluster_range);

                    if n_sub <= 1
                        cmap_full(cluster_range,:) = repmat(anchor_rgb,n_sub,1);
                        continue
                    end

                    % 🔥 Stronger contrast lightness envelope
                    % Split subgroup into dark → anchor → light
                    L_dark_min = 0.30;
                    L_dark_max = 0.55;
                    ...
                    half_point = ceil(n_sub/2);

                    % Darker side
                    dark_vals = linspace(L_dark_min, L_dark_max, half_point);

                    % Lighter side
                    light_vals = linspace(L_light_min, L_light_max, n_sub-half_point+1);
                    light_vals(1) = []; % avoid duplicate anchor region

                    L_vals = [dark_vals light_vals];

                    for k = 1:n_sub
                        hsv_temp = anchor_hsv;
                        hsv_temp(3) = L_vals(k);
                        cmap_full(cluster_range(k),:) = hsv2rgb(hsv_temp);
                    end
                end

                options.cmap = cmap_full;
            end
            ax = gca;
            if grouping_var == "E_IRatio"
                E_IRatio_order = [1,3,4,5,2];
